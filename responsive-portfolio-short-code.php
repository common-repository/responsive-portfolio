...
/*            box-shadow: 2px 2px 2px #ccc;
        ...
        <?php if($WL_Image_Border=="yes"){ ?> padding: 10px;
            background: <?php echo esc_attr($WL_Image_Border_Color);?>;
        <?php }  }?>
        }
        ...
        .weblizar-portfolio-gallery a div {
            ...
            background: rgba(0, 0, 0, <?php echo esc_attr($WL_Hover_Color_Opacity); ?>);
            width: 100%;
            height: 100%;
    ...
    <div class="gal-container">
		...
		if ( $WL_Gallery_Title == "yes" ) { ?>
            <!-- gallery title-->
            <div style="font-weight: bolder; text-align:center;font-size:16px; padding-bottom:20px; border-bottom:2px solid #f1f1f1; margin-bottom: 20px;font-family: <?php echo esc_attr($WL_Font_Style); ?>">
				<?php echo get_the_title( $post_id ); ?>
            </div>
		<?php } ?>
            <!-- gallery photos-->
            <ul class="weblizar-portfolio-grid effect-3" id="weblizar-portfolio-grid<?php echo esc_attr($post_id); ?>">
				<?php
				/**
				...
				foreach ( $rpg_all_photos_details as $rpg_single_photos_detail ) {
					...
					?>
                    <li class="<?php echo esc_attr($WL_Gallery_Layout); ?> col-sm-6 wl-gallery">
                        <div class="b-link-<?php echo esc_attr($WL_Hover_Animation); ?> b-animate-go">
                            <div id="da-thumbs">
								...
								<?php if ( $link ) { ?>
                                    <div class="showcase"><a href="<?php echo esc_url( $link ); ?>" target="_blank"><?php echo esc_html( $name ); ?></a>
                                    </div><?php } ?>
                            </div>
                        </div>
                    </li>
            ...
            <script type="text/javascript">
                ...
                jQuery(document).ready(function () {
                    new AnimOnScroll(document.getElementById('weblizar-portfolio-grid<?php echo esc_attr($post_id); ?>'), {
                        minDuration: 0.4,
                        maxDuration: 0.7,
                        viewportFactor: 0.2
                    });
